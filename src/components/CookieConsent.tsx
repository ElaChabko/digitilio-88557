      ...
      <div className="mx-auto max-w-5xl m-4 rounded-xl border bg-white text-slate-900 shadow-xl">
        <div className="p-4 md:p-6">
          ...
          <p className="text-sm leading-relaxed mb-4">
            Używamy niezbędnych cookies do prawidłowego działania serwisu. Dodatkowe
            cookies (analityczne/marketingowe) włączamy tylko za Twoją zgodą. Szczegóły w{" "}
            <a href="/polityka-cookies" className="underline">Polityce plików cookies</a> i{" "}
            <a href="/polityka-prywatnosci" className="underline">Polityce prywatności</a>.
          </p>
          ...
          <div className="grid gap-3 md:grid-cols-3">
            ...
            <label className="flex items-start gap-3 rounded-lg border p-3">
              ...
              <div className="text-sm">
                ...
                <div className="text-slate-600">Personalizacja reklam (jeśli wdrożysz).</div>
              </div>
            </label>
          </div>

          <div className="mt-4 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
            <button onClick={saveChoices} className="rounded-lg border px-4 py-2 text-sm hover:bg-slate-50">
              Zapisz wybór
            </button>
            <button onClick={acceptAll} className="rounded-lg bg-slate-900 px-4 py-2 text-sm text-white hover:bg-slate-800">
              Akceptuj wszystko
            </button>
          </div>

          <p className="mt-3 text-xs text-slate-500">Wersja zgód: {CONSENT_VERSION}</p>
        </div>
      </div>
    </div>
  );
